...
if($_SERVER["REQUEST_METHOD"] == "POST"){
            ...
            if (isset($_POST["row$i-column$j-title"])) { //get the title, if the column exists
                ...
                for ($k=0; $k < 2; $k++) {
                    if(isset($_FILES["row$i-column$j-image$k"])) {
                        ...
                        $file = "/../images/clipart-$title-$k$extension";
                    } else {
                        break;
                    }
    ...
    $titles_path = "/../clipart_posts/titles.json";
    if(fopen("/../clipart_posts/$file_name.json", "x")){
    ...
    //write the new file
    $file = fopen("/../clipart_posts/$file_name.json", "w");
    fwrite($file, json_encode($post));
    fclose($file);

    echo "file written to clipart_posts/$file_name.json<br>";
    echo "<a href=\".\">Back to Developer Index</a>";

}

else{
    echo "this page is meant to be shown after submitting the <a href=\"updateCLIPArt.html\">update clip art form</a>.";
}
